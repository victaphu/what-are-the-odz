...
import { Attachment, Message } from './MessageBubble'

setConfiguration({ maxScreenClass: 'xl' })

let reconnectID = 0;

const SendingMessage = (props: { lastMessage: Message; message: Message; nextMessage: Message }) => {
    ...
    function renderImages(attachments: Attachment[]) {
        return attachments.map((attachment, index) => {
            if (isImage(attachment.name)) {
                return <Thumbnail key={`attachment_${index}`} attachment={attachment} />
            } else {
    ...
    function renderFiles(attachments: Attachment[]) {
        return attachments.map((attachment, index) => {
            if (!isImage(attachment.name)) {
                return <FileView key={`attachment_${index}`} attachment={attachment} />
            } else {
                return <div key={`attachment${index}`} />
            }
        })
                ...
                <Col xs={12} sm={12} md={12}>
                    {
                        message.text &&
                        <div
                            className='ce-message-bubble ce-my-message-bubble float-right text-left'
                            style={{ ...styles.myMessage, ...{ borderRadius } }}
                        >
                            <Body text={message.text} myMessage={true} />
                        </div>
                    }
                </Col>
